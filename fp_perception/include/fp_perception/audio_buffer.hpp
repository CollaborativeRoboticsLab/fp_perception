...
{
public:
  static constexpr const char* kExpiredAudioSliceError = "error: requested audio slice is expired. for future "
                                                         "interactions, increase the audio retention window parameter.";
  ...
  void append(const audio_data& data, int max_duration_seconds, const rclcpp::Time& end_time)
  {
    ...
    if (buffer_.sample_rate != data.sample_rate || buffer_.channels != data.channels)
    {
      if (!buffer_.samples.empty())
      {
        RCLCPP_WARN(rclcpp::get_logger("AudioBuffer"),
                    "Resetting public audio buffer due to format change: sample_rate %d -> %d, channels %d -> %d, "
                    "buffered_samples=%zu",
    ...
    if (buffered_seconds < 0.5)
    {
      RCLCPP_DEBUG(rclcpp::get_logger("AudioBuffer"),
                   "Public audio buffer remains small after append: added_frames=%zu buffered_frames=%zu "
                   "buffered_seconds=%.3f retention=%d start=%.9f end=%.9f",
  ...
  void waitForAudio(std::chrono::seconds timeout = std::chrono::seconds(5))
  {
    ...
    if (!isInitializedLocked())
      throw fp_perception_exception("public audio buffer not initialized");
  }
  ...
  audio_data readLatest(int duration_seconds)
  {
    ...
    if (available_samples == 0)
      throw fp_perception_exception("public audio buffer is empty");
  ...
  audio_data readWindow(const rclcpp::Time& start_time, int duration_seconds)
  {
    ...
    if (!initialized_time_ || buffer_.samples.empty())
      throw fp_perception_exception("public audio buffer timestamp state is not initialized");
                     ...
                     std::chrono::seconds timeout = std::chrono::seconds(30))
  {
    ...
    if (isExpiredLocked(start_time))
    {
      ...
      throw fp_perception_exception(
          std::string(kExpiredAudioSliceError) + " requested=[" + std::to_string(start_time.seconds()) + ", " +
          std::to_string(requested_end_time.seconds()) + "] buffered=[" + std::to_string(buffer_start_time_.seconds()) +
    ...
    if (currentEndTimeLocked() < requested_end_time)
    {
      const auto current_end_time = currentEndTimeLocked();
      throw fp_perception_exception(
          "Timeout waiting for requested audio slice. requested=[" + std::to_string(start_time.seconds()) + ", " +
          std::to_string(requested_end_time.seconds()) + "] buffered=[" + std::to_string(buffer_start_time_.seconds()) +
          ", " + std::to_string(current_end_time.seconds()) + "]");
    }
  }
...
private:
  ...
  static size_t durationToFrames(const rclcpp::Duration& duration, int sample_rate)
  {
    ...
    return static_cast<size_t>(
        (static_cast<long double>(duration.nanoseconds()) * static_cast<long double>(std::max(1, sample_rate))) /
        1000000000.0L);
  }
