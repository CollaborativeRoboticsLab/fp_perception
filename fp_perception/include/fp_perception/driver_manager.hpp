...
{
public:
  DriverManager(const rclcpp::Node::SharedPtr& node, pluginlib::ClassLoader<fp_perception::DriverBase>& loader)
                                         ...
                                         const std::string& interface_description) const
  {
    ...
    if (!typed_driver)
    {
      throw fp_perception_exception("Loaded " + driver_description + " does not implement " + interface_description +
                                    ": " + plugin_name);
    }
...
private:
  ...
  pluginlib::ClassLoader<fp_perception::DriverBase>& loader_;
};
