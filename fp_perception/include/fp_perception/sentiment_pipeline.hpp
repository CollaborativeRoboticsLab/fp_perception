...
public:
  ...
  SentimentPipeline(std::shared_ptr<SentimentAnalysisDriver> sentiment_driver,
                    std::shared_ptr<TranscriptionDriver> transcription_driver, AudioReader readDeviceAudio)
  ...
  sentiment_result run(sentiment_request request)
  {
    if (!sentiment_driver_)
      throw fp_perception_exception("Sentiment driver is not loaded.");
    ...
    if (request.use_device_audio)
    {
      ...
      if (!transcription_result_data.success)
        throw fp_perception_exception(transcription_result_data.error.empty() ? "No transcription result available for "
                                                                                "sentiment analysis." :
