 ...
 * @throws fp_perception_exception if the output stream is not valid or if there are issues writing the data.
 */
void writeWavStream(std::ostream& out, const audio_data& data)
{
  if (!out)
    throw fp_perception_exception("Output stream is not valid");
 ...
 * @param data The audio data to write.
 *  @throws fp_perception_exception if the file cannot be opened or if there are issues writing the data.
 */
 ...
 * @throws fp_perception_exception on file errors or unsupported formats.
 */
audio_data readWavFile(const std::string& filepath)
{
  std::ifstream in(filepath, std::ios::binary);
  if (!in)
    throw fp_perception_exception("Failed to open WAV file: " + filepath);

  char riff_header[4];
  in.read(riff_header, 4);
  if (std::strncmp(riff_header, "RIFF", 4) != 0)
    throw fp_perception_exception("Invalid WAV file: Missing 'RIFF'");

  in.seekg(20);
  uint16_t audio_format = 0;
  in.read(reinterpret_cast<char*>(&audio_format), 2);
  if (audio_format != 1)
    throw fp_perception_exception("Unsupported WAV format: Only PCM is supported");
  ...
  if (bits_per_sample != 16)
    throw fp_perception_exception("Unsupported bit depth: Only 16-bit PCM is supported");
  ...
  if (std::strncmp(chunk_id, "data", 4) != 0)
    throw fp_perception_exception("Invalid WAV file: Missing 'data' chunk");
 ...
 * @throws fp_perception_exception if there are issues writing the data.
 */
