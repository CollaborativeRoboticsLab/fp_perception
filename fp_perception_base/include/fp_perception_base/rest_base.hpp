...
#include <fp_perception_base/rest/structs.hpp>
#include <fp_perception_base/rest/file_part_source.hpp>
 ...
 * @brief RestBase
 *
 * This class implements a base class for RESTful fp_perception plugins.
...
public:
   ...
   * @throws fp_perception_exception if there is an error during initialization
   */
                                    ...
                                    std::string api_key_name = "")
  {
    ...
    if (!api_key_name.empty())
    {
      ...
      else
      {
        ...
        throw fp_perception::fp_perception_exception("missing env variable: " + api_key_name);
        api_key_ = "";
      }
    }
    else
   ...
   * @throws fp_perception_exception if there is an error during the request.
   */
  virtual fp_perception::RESTResponse call(const fp_perception::RESTRequest& req)
  {
    ...
    if (!curl)
    {
      ...
      throw fp_perception::fp_perception_exception("Failed to initialize libcurl");
    }
    ...
    else
    {
      ...
      throw fp_perception::fp_perception_exception("Unsupported auth type: " + auth_type_);
    }
    ...
    if (res != CURLE_OK)
    {
      ...
      throw fp_perception::fp_perception_exception("cURL error: " + std::string(curl_easy_strerror(res)));
    }
    ...
    if (http_code != 200)
    {
      ...
      throw fp_perception::fp_perception_exception(msg);
    }
    ...
    catch (const std::exception& e)
    {
      ...
      throw fp_perception::fp_perception_exception("JSON parse error: " + std::string(e.what()));
    }
  }
  ...
  virtual fp_perception::RESTResponse call_audio(const fp_perception::RESTRequest& req)
  {
    ...
    {
      record_rest_result(false, 0, "Failed to initialize libcurl");
      throw fp_perception_exception("Failed to initialize libcurl");
    }
    ...
    if (!mime)
    {
      ...
      throw fp_perception_exception("Failed to initialize libcurl MIME form");
    }
    ...
    for (const auto& opt : req.options)
    {
          ...
          curl_mime_data(option_part, opt.value.c_str(), CURL_ZERO_TERMINATED) != CURLE_OK)
      {
        ...
        throw fp_perception_exception("Failed to build MIME field for option: " + opt.key);
      }
    }
        ...
        curl_mime_type(file_part, req.file_type.c_str()) != CURLE_OK)
    {
      ...
      throw fp_perception_exception("Failed to build MIME audio payload");
    }
    ...
    else
    {
      ...
      curl_easy_cleanup(curl);
      record_rest_result(false, 0, "Unsupported auth type: " + auth_type_);
      throw fp_perception_exception("Unsupported auth type: " + auth_type_);
    }
    ...
    if (res != CURLE_OK)
    {
      ...
      throw fp_perception_exception("cURL error: " + std::string(curl_easy_strerror(res)));
    }
    ...
    if (http_code != 200)
    {
      const std::string msg = build_http_error_message(http_code, response_data);
      record_rest_result(false, http_code, msg);
      throw fp_perception_exception(msg);
    }
    ...
    {
      record_rest_result(false, http_code, std::string("JSON parse error: ") + e.what());
      throw fp_perception_exception("JSON parse error: " + std::string(e.what()));
    }
  }
  ...
  virtual fp_perception::RESTResponse call_tts(const fp_perception::RESTRequest& req)
  {
    ...
    fp_perception::RESTResponse response;
...
protected:
   ...
   *
   * This method converts the fp_perception request to a JSON object that can be sent to the fp_perception plugin.
   * It includes the prompt text and options for the JSON object.
   *
   * @param prompt The fp_perception request to convert
  ...
  /**
   * @brief Convert a JSON object to a fp_perception response
   *
   * This method converts a JSON object received from the fp_perception plugin into a fp_perception response.
   * It extracts the relevant fields from the JSON object and returns a fp_perception response object.
   *
   * @param object The JSON object to convert
   * @return A fp_perception response object containing the response data
   */
