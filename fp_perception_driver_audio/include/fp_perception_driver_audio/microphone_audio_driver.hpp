...
public:
  ...
  void initialize(const rclcpp::Node::SharedPtr& node) override
  {
    ...
    if (!device_name_.empty())
    {
      try
      {
        ...
        RCLCPP_INFO(node_->get_logger(), "Resolved microphone device_name '%s' to device_id %d.", device_name_.c_str(),
                    resolved_device_id);
      }
    ...
    {
      RCLCPP_WARN(node_->get_logger(), "Microphone device id %d was replaced by %d (%s).", device_id_,
                  resolved_device_id, device_info->name);
    ...
    if (err != paNoError)
    {
      ...
      throw fp_perception_exception("Failed to open microphone stream: " + std::string(Pa_GetErrorText(err)));
    }
    ...
    if (err != paNoError)
    {
      ...
      throw fp_perception_exception("Failed to start microphone stream: " + std::string(Pa_GetErrorText(err)));
    }
   ...
   * audio data from the microphone stream and returns it as a fp_perception::audio_data. Best
   ...
   *
   * @return fp_perception::audio_data The latest audio chunk(s) from the driver.
  ...
  audio_data readChunk() override
  {
    ...
    else
    {
      ...
      {
        throw fp_perception_exception("Timeout waiting for audio data.");
      }
    }

    if (buffered_sample_count_ < chunk_samples && !is_running_)
      throw fp_perception_exception("Microphone stream is not running.");
  ...
  void test() override
  {
    ...
    const double rms = accumilated_samples.samples.empty() ?
                           0.0 :
                           std::sqrt(static_cast<double>(sum_squares / accumilated_samples.samples.size()));
    ...
    if (max_abs < 128)
    {
      RCLCPP_WARN(node_->get_logger(), "Microphone test appears nearly silent. Check capture device_id, input source, "
                                       "gain, mute state, and channel count.");
    }
...
protected:
  ...
  int capture_input_buffer(const void* input_buffer, unsigned long frames_per_buffer,
