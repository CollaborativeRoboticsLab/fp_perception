...
#include <fp_perception_base/audio/wav.hpp>
#include <fp_perception_driver_audio/utils.hpp>
...
public:
  ...
  void initialize(const rclcpp::Node::SharedPtr& node) override
  {
    ...
    if (err != paNoError)
    {
      ...
      throw fp_perception_exception("PortAudio initialization failed: " + std::string(Pa_GetErrorText(err)));
    }
    ...
    if (device_count < 0)
    {
      ...
      throw fp_perception_exception("PortAudio failed to enumerate devices");
    }
    ...
    if (!device_name_.empty())
    {
      try
      {
        ...
        RCLCPP_INFO(node_->get_logger(), "Resolved speaker device_name '%s' to device_id %d.", device_name_.c_str(),
                    resolved_device_id);
      }
      catch (const fp_perception_exception& e)
      {
    ...
    if (!device_info)
    {
      throw fp_perception_exception("PortAudio returned null device info for device_id " +
                                    std::to_string(resolved_device_id));
    }
    ...
    {
      throw fp_perception_exception("Selected device_id " + std::to_string(device_id_) + " ('" + device_info->name +
  ...
  void play_internal(const audio_data& input_data, bool wait_for_drain)
  {
    ...
    if (data.samples.empty())
    {
      ...
      throw fp_perception_exception("Received empty audio data, nothing to set.");
    }
    ...
    if (stream_dict_.find(stream_key) == stream_dict_.end())
    {
      ...
      if (err != paNoError)
      {
        ...
        throw fp_perception_exception("Failed to open speaker stream: " + std::string(Pa_GetErrorText(err)));
      }
      ...
      if (err != paNoError)
      {
        ...
        throw fp_perception_exception("Failed to start speaker stream: " + std::string(Pa_GetErrorText(err)));
      }

      if (!Pa_IsStreamActive(stream))
      {
        RCLCPP_ERROR(node_->get_logger(), "Stream is not active after starting.");
        throw fp_perception_exception("PortAudio stream failed to activate.");
      }
    ...
    try
    {
      if (wait_for_drain)
      {
        RCLCPP_INFO(node_->get_logger(), "Queueing audio to speaker stream %s and waiting for playback drain.",
                    stream_key.c_str());
      }
      else
      {
        RCLCPP_INFO(node_->get_logger(), "Queueing audio to speaker stream %s without waiting for playback drain.",
                    stream_key.c_str());
      }
  ...
  void test() override
  {
    ...
    try
    {
      ...
      const double rms =
          audio_data.samples.empty() ? 0.0 : std::sqrt(static_cast<double>(sum_squares / audio_data.samples.size()));
      const double duration =
          static_cast<double>(audio_data.samples.size()) /
          static_cast<double>(std::max(1, audio_data.sample_rate) * std::max(1, audio_data.channels));
      RCLCPP_INFO(node_->get_logger(), "Speaker test input stats: max_abs=%d rms=%.2f estimated_duration=%.2fs",
                  max_abs, rms, duration);
      if (max_abs < 128)
      {
        RCLCPP_WARN(node_->get_logger(), "Speaker test input appears nearly silent; use a known-good WAV to test "
                                         "output routing.");
      }
...
protected:
                                                          ...
                                                          int input_rate, int output_rate)
  {
    if (input_rate <= 0 || output_rate <= 0)
      throw fp_perception_exception("Invalid sample rate for resampling");
  ...
  void queue_data(audio_data input_data, const std::string& stream_key, bool wait_for_drain)
  {
    ...
    if (input_data.channels != channels_)
    {
      ...
      {
        throw fp_perception_exception("Unsupported channel conversion from " + std::to_string(input_data.channels) +
    ...
    {
      throw fp_perception_exception("Insufficient data" + std::to_string(data.size()) + " for requested chunk size " +
