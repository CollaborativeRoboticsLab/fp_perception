 ...
 * @throws fp_perception::fp_perception_exception if no devices are found or if the target device is not found.
 */
...
inline int getDeviceIdByName(const std::string& target_name)
{
  ...
  if (num_devices < 0)
    throw fp_perception::fp_perception_exception("No PortAudio devices found");
  ...
  throw fp_perception::fp_perception_exception("Device with name \"" + target_name + "\" not found");
}
}  // namespace fp_perception
