...
`fp_perception_driver_audio` provides audio input and output drivers for ROS2 using the [PortAudio](http://www.portaudio.com/) library. It defines two main drivers built on a shared `DriverBase`:
...
These drivers are intended to be used within the `fp_perception` ROS2 package and integrate with the event logging and configuration system defined by the `DriverBase` interface.
...
```cpp
auto driver = std::make_shared<fp_perception::MicrophoneAudioDriver>();
...
```cpp
fp_perception::SpeakerAudioDriver speaker;
