...
public:
  ...
  image_analysis_result analyze(const image_analysis_request& request_data) override
  {
    ...
    if (frame.empty())
      throw fp_perception_exception("OpenAIImageAnalysisDriver received an empty image frame");

    std::vector<uchar> png_bytes;
    if (!cv::imencode(".png", frame, png_bytes))
      throw fp_perception_exception("OpenAIImageAnalysisDriver failed to encode image to PNG");
  ...
  void test() override
  {
    ...
    if (image.empty())
      throw fp_perception_exception("Failed to read test image file: " + filepath.string());
    ...
    {
      throw fp_perception_exception(analysis.error.empty() ? "No image analysis result received." : analysis.error);
    }
  }
...
protected:
  ...
  virtual nlohmann::json toJson(const fp_perception::RESTRequest& request)
  {
    if (request.file_stream.empty())
      throw fp_perception_exception("Image analysis request missing image payload");
