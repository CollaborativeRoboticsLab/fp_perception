...
public:
  ...
  void initialize(const rclcpp::Node::SharedPtr& node) override
  {
    ...
    chunk_max_words_ = std::max<size_t>(
        1, static_cast<size_t>(node->get_parameter("driver.speech.OpenAISpeechDriver.chunk_max_words").as_int()));
    chunk_parallel_requests_ = std::max<size_t>(
        1,
        static_cast<size_t>(node->get_parameter("driver.speech.OpenAISpeechDriver.chunk_parallel_requests").as_int()));
  ...
  audio_data synthesize(const text_data& new_text) override
  {
    ...
    else
    {
      for (size_t begin = 0; begin < chunks.size(); begin += chunk_parallel_requests_)
      {
        ...
        std::vector<std::future<fp_perception::RESTResponse>> futures;
  ...
  void test() override
  {
    ...
    fp_perception::text_data new_text;
    ...
    else
    {
      ...
      throw fp_perception_exception("No speech synthesis result received.");
    }
...
protected:
  ...
  fp_perception::RESTResponse synthesize_chunk(const std::string& chunk, const std::string& voice,
                                               const std::string& instructions)
  {
    ...
    fp_perception::RESTOption model_option1;
    model_option1.key = "model";
    model_option1.value = model_name_;
    model_option1.type = fp_perception::RESTOptionType::STRING;
    request.options.push_back(model_option1);

    fp_perception::RESTOption model_option2;
    model_option2.key = "response_format";
    model_option2.value = "pcm";
    model_option2.type = fp_perception::RESTOptionType::STRING;
    request.options.push_back(model_option2);

    fp_perception::RESTOption model_option3;
    model_option3.key = "voice";
    model_option3.value = voice;
    model_option3.type = fp_perception::RESTOptionType::STRING;
    request.options.push_back(model_option3);

    fp_perception::RESTOption model_option4;
    model_option4.key = "instructions";
    model_option4.value = instructions;
    model_option4.type = fp_perception::RESTOptionType::STRING;
  ...
  {
    (void)object;
    throw fp_perception_exception("fromJson() not implemented for this driver.");
  }
