...
public:
   ...
   * audio chunks in the form of fp_perception::audio_data.
   *
  ...
  transcription_result transcribe(const transcription_request& request_data) override
  {
    ...
    // create fp_perception::RESTRequest object
    ...
    RCLCPP_INFO(node_->get_logger(), "Completed OpenAI transcription request: response_chars=%zu",
  ...
  void test() override
  {
    ...
    if (data.samples.empty())
    {
      ...
      throw fp_perception_exception("Failed to read test audio file.");
    }
    ...
    else
    {
      ...
      throw fp_perception_exception(transcription.error.empty() ? "No transcription result received." :
                                                                  transcription.error);
    }
...
protected:
  ...
  {
    (void)request;
    throw fp_perception_exception("toJson() not implemented for this driver.");
  }
