...
public:
     ...
     */
    static GazeMetrics calculateAttentionMetrics(
        const std::vector<std::pair<double, bool>>& window,
        double interval_duration = 3.0)
    {
        GazeMetrics metrics;

        if (window.empty()) return metrics;
        ...
        for (const auto& [_, state] : filtered_window) {
            if (state) robot_looks++;
        }

        int non_robot_looks = frames_in_interval - robot_looks;
        double attention_ratio = (frames_in_interval > 0)
            ? static_cast<double>(robot_looks) / frames_in_interval
            : 0.0;
        ...
        if (frames_in_interval > 0) {
            ...
            if (p_robot > 0) gaze_entropy -= p_robot * std::log2(p_robot);
            if (p_non_robot > 0) gaze_entropy -= p_non_robot * std::log2(p_non_robot);
        }
